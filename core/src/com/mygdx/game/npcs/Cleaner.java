...
import static com.mygdx.game.npcs.Repairman.loan;
import static com.mygdx.game.npcs.Repairman.loanFrequency;
...
public class Cleaner extends Actor{
    ...
    public static Texture t = new Texture("cleaner.png");
    // textures for the different directions the player can face
    private final Texture rightt = new Texture("+cleaner.png");
    private final Texture leftt = new Texture("-cleaner.png");
    private final Texture centert = new Texture("cleaner.png");
    private final Texture backt = new Texture("cleanerback.png");
    ...
    public static int loan = 150;
    public static int loanFrequency = 90;
    
    public static double costpermin = ((float)loan*60f/(float)loanFrequency);
    
    public Cleaner(Graph graph, World world){
        this.graph = graph;
    ...
    public void act(float delta) {
        ...
        if(getGoal() != null && getGoal().getBuildingId() != 31){
            Node trash = graph.findId(31, xGrid, yGrid);
            if(trash != null){    
        ...
        if(getPath() == null || getGoal() == null){
            ...
            if(getGoal() != null && getPath() != null){
                ...
                if(Math.sqrt(Math.pow(x1-x2, 2) + Math.pow(y1-y2, 2)) < 5){
                    if(getGoal().getBuildingId() == 31){
                        world.getGrid()[getGoal().getX()][getGoal().getY()] = MapElement.getNewMapElementById(30);
                    }
                    setGoal(null);
                    setPath(null);
                    setPathPart(0);
